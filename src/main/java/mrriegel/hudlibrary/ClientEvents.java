...
public class ClientEvents {
	...
	private static Cache<DirectionPos, List<HUDElement>> cachedElements = CacheBuilder.newBuilder().maximumSize(100).expireAfterWrite(250, TimeUnit.MILLISECONDS).build();
	private static Cache<Integer, Integer> glIndexes = CacheBuilder.newBuilder().maximumSize(100).expireAfterWrite(10, TimeUnit.MINUTES).removalListener(n -> GLAllocation.deleteDisplayLists((int) n.getValue())).build();
	private static Cache<Integer, Boolean> glIndexes2 = CacheBuilder.newBuilder().maximumSize(100).expireAfterWrite(200, TimeUnit.MILLISECONDS).build();

	public static Set<TileEntity> hudTiles = Collections.newSetFromMap(new IdentityHashMap<>());

	@SubscribeEvent
	public static void attach(AttachCapabilitiesEvent<TileEntity> event) {
		if (FMLCommonHandler.instance().getEffectiveSide().isClient())
			hudTiles.add(event.getObject());
	}
	...
	public static void render(RenderWorldLastEvent event) {
		...
		//		useList = true;
		hudTiles.removeIf(t -> t.isInvalid() || !t.hasCapability(HUDCapability.cap, null));
		List<TileEntity> l = null;
		try {
			l = hudTiles/*mc.world.loadedTileEntityList*/.stream().filter(t -> {
